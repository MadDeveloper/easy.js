...
function RoutingController(SkeletonFactory) {
    ...
    return {
        isRequestWellParameterized: function isRequestWellParameterized() {
            ...
            return Controller.verifyParams([{ property: 'example', typeExpected: 'string' }], Request.getBody());
        },

        getSkeletons: function getSkeletons() {
            SkeletonRepository.readAll().then(function (skeletons) {
        ...
        createSkeleton: function createSkeleton() {
            if (this.isRequestWellParameterized()) {

                database.transaction(function (t) {

                    SkeletonRepository.save(SkeletonFactory.getNewModel(), Request.getBody(), { transacting: t }).then(function (skeleton) {
        ...
        updateSkeleton: function updateSkeleton() {
            if (this.isRequestWellParameterized()) {

                database.transaction(function (t) {

                    SkeletonRepository.save(Request.find('skeleton'), Request.getBody(), { transacting: t }).then(function (skeleton) {
        ...
        patchSkeleton: function patchSkeleton() {
                                ...
                                switch (patch.op) {
                                    case 'replace':
                                        if (_.indexOf(validPaths, patch.path) >= 0) {
                                            SkeletonRepository.patch(Request.find('skeleton'), patch, { transacting: t, patch: true }).then(function (skeleton) {
        ...
        deleteSkeleton: function deleteSkeleton() {
            database.transaction(function (t) {

                SkeletonRepository.delete(Request.find('skeleton'), { transacting: t }).then(function () {
